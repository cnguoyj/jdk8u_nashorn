...
import static jdk.nashorn.internal.asm.Opcodes.ATHROW;
...
import static jdk.nashorn.internal.asm.Opcodes.GOTO;
...
import static jdk.nashorn.internal.asm.Opcodes.IFNULL;
...
import static jdk.nashorn.internal.asm.Opcodes.INSTANCEOF;
...
public class MethodEmitter {
    ...
    MethodEmitter loadType(final String className) {
        ...
        method.visitLdcInsn(jdk.nashorn.internal.asm.Type.getObjectType(className));
    ...
    private static class LocalVariableDef {
        ...
        private final jdk.nashorn.internal.asm.Label label;
        // The currently live type of the local variable.
        private final Type type;

        LocalVariableDef(final jdk.nashorn.internal.asm.Label label, final Type type) {
            this.label = label;
            this.type = type;
        }
    ...
    private void endLocalValueDef(final Symbol symbol, final LocalVariableDef def, final jdk.nashorn.internal.asm.Label label) {
    ...
    void store(final Symbol symbol, final Type type, final boolean onlySymbolLiveValue) {
        ...
        if (symbol.hasSlot()) {
            ...
            if(existingDef == null || existingDef.type != type) {
                final jdk.nashorn.internal.asm.Label here = new jdk.nashorn.internal.asm.Label();
                if(isLiveType) {
    ...
    static jdk.nashorn.internal.asm.Label[] getLabels(final Label... table) {
        final jdk.nashorn.internal.asm.Label[] internalLabels = new jdk.nashorn.internal.asm.Label[table.length];
    ...
    void lineNumber(final int line) {
        if (context.getEnv()._debug_lines) {
            ...
            final jdk.nashorn.internal.asm.Label l = new jdk.nashorn.internal.asm.Label();
            method.visitLabel(l);
